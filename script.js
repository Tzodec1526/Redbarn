...
// ---- Palette toggle (ink is primary; cream is the light option) ----
(function(){
  ...
  // First visit / no preference → ink
  if (!document.documentElement.dataset.palette) {
    try {
      if (localStorage.getItem('redbarn-palette') !== 'cream') {
  ...
  toggle.addEventListener('click', () => {
    const isInk = document.documentElement.dataset.palette === 'ink';
    const next = isInk ? 'cream' : 'ink';
    if (next === 'cream') delete document.documentElement.dataset.palette;
    else document.documentElement.dataset.palette = 'ink';
...
// ---- JS sentinel ----
// Added last, after the observers above are wired up: .reveal is only hidden
// once the code that un-hides it is guaranteed to be running. If this file
// fails to load or throws earlier, content stays visible (unanimated).
